...
function climbingLeaderboard(ranked, player) {
    ...
    let maxRanked = ranks[0];
    let minRanked = ranks[ranks.length-1];
    let score;
    let res = [];
    let j = 0;
    for (let i = 0; i < player.length; i++) {
        ...
        if(score >= maxRanked) {
            res.push(1);
        } else if(score <= minRanked) {
            res.push(ranks.length);
        } else if(score < maxRanked && score > minRanked) {
